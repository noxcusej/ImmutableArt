...
function useOnScroll(el, options, wait) {
  ...
  const methods = SCROLL_METHODS.reduce((p, c) => {
    p[c] = (...args) => scrollableElement.value && scrollableElement.value[c] && scrollableElement.value[c].apply(scrollableElement.value, args);
    return p;
  }, {});
...
if (true) {
  try {
    setupDevtoolsPlugin = (init_esm(), esm_exports).setupDevtoolsPlugin;
  } catch {
  }
}
...
function isQuotaExceededError(e, storage) {
  return e instanceof DOMException && (e.code === 22 || e.code === 1014 || e.name === "QuotaExceededError" || e.name === "NS_ERROR_DOM_QUOTA_REACHED") && (storage && storage.length !== 0 || false);
}
...
function useUndo(defaultValue, options) {
  ...
  if (options && options.devtoolId) {
    const layer = useDevtoolsTimelineLayer(`useUndo:${options.devtoolId}`, options.devtoolId, 3318463);

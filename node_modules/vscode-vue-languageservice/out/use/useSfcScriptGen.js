...
function useSfcScriptGen(lsType, ts, vueUri, vueDoc, script, scriptSetup, scriptAst, scriptSetupAst, sfcTemplateCompileResult, sfcStyles) {
    let version = 0;
    const scriptRanges = (0, reactivity_1.computed)(() => scriptAst.value
        ? (0, scriptRanges_1.parseScriptRanges)(ts, scriptAst.value, !!scriptSetup.value)
        : undefined);
    ...
    const codeGen = (0, reactivity_1.computed)(() => (0, script_1.generate)(lsType, vueUri, script.value, scriptSetup.value, scriptRanges.value, scriptSetupRanges.value, () => htmlGen.value, () => sfcStyles.value));
    ...
    const textDocument = (0, reactivity_1.computed)(() => {
        return vscode_languageserver_textdocument_1.TextDocument.create(lsType === 'template' ? `${vueUri}.__VLS_script.${lang.value}` : `${vueUri}.${lang.value}`, shared.syntaxToLanguageId(lang.value), version++, codeGen.value.getText());
    });

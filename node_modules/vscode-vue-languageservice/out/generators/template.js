...
function generate(sourceLang, templateAst, isVue2, cssScopedClasses = [], htmlToTemplate, isScriptSetup) {
    ...
    for (const name of usedComponents) {
        ...
        if (tagResolved) {
            tsCodeGen.addText('// @ts-ignore\n');
            tsCodeGen.addText(`${tagResolved.emit}('${string_1.SearchTexts.EmitCompletion(name)}');\n`);
        }
    }
    ...
    for (const name of usedComponents) {
        const tagResolved = tagResolves[name];
        if (tagResolved) {
            tsCodeGen.addText(`${tagResolved.baseProps}.${string_1.SearchTexts.PropsCompletion(name)};\n`);
        }
    }
    for (const childNode of templateAst.children) {
        tsCodeGen.addText(`{\n`);
